...
async function route(request, env) {
  ...
  if (url.pathname === '/api/youtube-feed' && request.method === 'GET') {
    // Read-only public lookup, same reasoning as /api/pay's GET - generous
    // limit, mainly there to blunt scraping rather than protect a real
    // customer's usage.
    const ip = request.headers.get('CF-Connecting-IP') || 'unknown';
    const { allowed, retryAfterSeconds } = await checkRateLimit(env, `youtube-feed:${ip}`, { limit: 60, windowSeconds: 600 });
    if (!allowed) {
      return new Response(JSON.stringify({ error: 'Too many requests.' }), {
        status: 429,
        headers: { 'Content-Type': 'application/json', 'Retry-After': String(retryAfterSeconds) }
    ...
    try {
      return await handleYoutubeFeed(request, env);
    } catch (err) {
